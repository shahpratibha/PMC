...
document.addEventListener('click', function(event) {
  if (!event.target.closest('.filter-group') && !event.target.closest('#toggleFilters')) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  button.addEventListener('click', function () {
    if (!filtersVisible) {
      filters.classList.add('active'); // Show the filter
      button.innerHTML = '<i class="fa-solid fa-filter-circle-xmark"></i>';
    } else {
      filters.classList.remove('active'); // Hide the filter
...
function DataTableFilter(cql_filter1) {
  ...
  var headers = ['Work_ID', 'Name_of_Work', 'Department', 'Budget_Code', 'Work_Type', 'Name_of_JE', 'Agency', 'stage', 'Tender_Amount', 'Created_At','Status'];
