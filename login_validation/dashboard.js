...
//           // Generate the URL with Work_ID
//           let qrURL = `http://192.168.1.8/IWMS_test2/login1.php`;
...
//       // Store Work_ID in session using AJAX
//       if (workID) {
//         $.post('set_work_id.php', { Work_ID: workID }, function(response) {
//           console.log('Work_ID set in session:', response);
//         });
//       }
...
//       // let qrURL = `http://192.168.1.8/IWMS_test2/login1.php`;
//       let qrURL = ` http://localhost/IWMS_test2/login1.php`;
...
map.on("contextmenu", async (e) => {
  ...
  for (let layer in layerDetails) {
    ...
    try {
      ...
      // Store Work_ID in local storage
      if (workID) {
      ...
      document.getElementById('generateQR').addEventListener('click', () => {
        ...
        // Add Work_ID to the login form
        let loginForm = document.querySelector('.login-form');
        if (loginForm) {
          let workIDField = document.createElement('input');
          workIDField.type = 'hidden';
          workIDField.name = 'Work_ID';
          workIDField.value = workID;
          loginForm.appendChild(workIDField);
        }
      });
...
//                   if (qrCanvas) {
//                       let qrImage = qrCanvas.toDataURL("image/png");
//                       let whatsappURL = `https://wa.me/?text=${encodeURIComponent(qrData)}&media=${encodeURIComponent(qrImage)}`;
//                       window.open(whatsappURL, '_blank');
//                   }
//               });

              
//               let loginUrl = `login1.php?qrData=${encodeURIComponent(qrData)}`;
...
//       } catch (error) {
//           console.error("डेटा मिळवण्यात त्रुटी:", error);
//       }
//   }
// });
