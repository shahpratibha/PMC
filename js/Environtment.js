...
var ward_admin_boundary = L.tileLayer
...
let ward_ids = ward_id
  ? ward_id.split(",").filter((id) => id && id !== "null")
  : [];
let zone_ids = zone_id
  ? zone_id.split(",").filter((id) => id && id !== "null")
  : [];
let prabhag_ids = prabhag_id
  ? prabhag_id.split(",").filter((id) => id && id !== "null")
  : [];
...
if (ward_ids.length > 0) {
  cql_filterm += ` AND ward_id IN(${ward_ids
    .map((id) => `'${id}'`)
    .join(",")})`;
} else {
...
if (prabhag_ids.length > 0) {
  cql_filterm += ` AND prabhag_id IN(${prabhag_ids
    .map((id) => `'${id}'`)
    .join(",")})`;
}
