...
var northArrowControl = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    container.innerHTML =
      '<img  src="png/002-cardinal-point.png" class=" north-arrow-img border-0;" alt="" style="width: 27px;  height:36px; background-color: white; border:2px solid darkblue; ">';
    return container;
  },
});
...
// Function to close the legend if clicked outside
function closeLegendOnClickOutside(event) {
  var legendDiv = document.querySelector(".info.legend");
  var button = document.querySelector(".collapse-button");

  // Check if the click is outside the legend and button
  if (legendVisible && !legendDiv.contains(event.target) && !button.contains(event.target)) {
    legendDiv.style.display = "none";
    button.innerHTML = "<i class='fa-solid fa-list' style='color:darkblue;'></i>";
    button.style.backgroundColor = "white"; // Change color to indicate action
    legendVisible = false;
...
document.addEventListener("click", closeLegendOnClickOutside);
...
function handleMediaQueryChange(mediaQuery) {
  ...
  // Check if the media query matches for mobile devices
  if (mediaQuery.matches) {
    // Media query matches for mobile (max-width: 767px)
    button.onclick = function () {
      if (legendDiv.style.height === "0px" || legendDiv.style.display === "none") {
        legendDiv.style.display = "block";
        legendDiv.style.height = "40vh";
        legendDiv.style.width = "40%"; // Adjust width to 40% for mobile view
        legendDiv.style.top = "12%";
        legendDiv.style.right = "40%"; // Adjust right to 40% for mobile view
        legendDiv.style.scrollbarWidth = "thin";
        legendDiv.style.scrollbarColor = "#163140 white";
        legendDiv.style.borderRadius = "10px";
        legendDiv.style.boxShadow = "5px 5px 5px rgba(0, 0, 0, 0.7)"; // Add shadow
    ...
    // Logic for other screen sizes, if needed
  }
}
