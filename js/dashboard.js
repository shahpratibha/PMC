...
$(document).ready(function () {
  ...
  function loadinitialData(cql_filter) {
    ...
    filternames.forEach(function (filtername) {
      ...
      $.getJSON(url, function (data) {
        ...
        $.each(data.features, function (index, feature) {
          ...
          if (column_name !== null && column_name !== "#N/A") {
            if (projectFiMap.has(column_name)) {
              ...
              projectFiMap.set(
                column_name,
                (projectFiMap.get(column_name) || 0) + 1
              );
            } else {
        ...
        var uniqueProjectFiList = Array.from(projectFiMap.entries()).map(
  ...
  function initialize() {
    $("#daterange").on("apply.daterangepicker", function (ev, picker) {
      ...
      console.log(
        "Selected date rangelooooooooooooooo:",
        startDate,
        "to",
        endDate
      );
...
function populateDropdown(dropdownId, data) {
  ...
  data.forEach(function (item) {
    ...
    var listItem = $(
      '<li><label><input type="checkbox" class="select2-option-checkbox" value="' +
        item +
        '"> ' +
        item +
        "</label></li>"
    ...
    function renderPageButtons(startPage) {
      ...
      prevButton.addEventListener("click", function () {
        if (currentPage > 1) {
          ...
          renderPageButtons(
            Math.max(1, currentPage - Math.floor(buttonsToShow / 2))
          );
        }
      });
  ...
  function createTable(data, headers) {
    ...
    data.forEach((item, index) => {
      ...
      headers.slice(1).forEach((header) => {
        // Exclude the first header (Serial No)
    ...
    input.addEventListener("input", function () {
      ...
      for (i = 0; i < arr.length; i++) {
        if (arr[i].toLowerCase().includes(val)) { // Check if the suggestion contains the input value
          ...
          console.log(
            "Adding IWMS_point, IWMS_line, and IWMS_polygon layers with filter:",
            cqlFilter
          );
          IWMS_point.addTo(map).bringToFront();
          IWMS_line.addTo(map).bringToFront();
  ...
  input.addEventListener("keydown", function (e) {
    ...
    if (e.keyCode === 40) {
      ...
      if (currentFocus > -1) {
        if (x) {
          selectedValue =
            x[currentFocus].getElementsByTagName("input")[0].value; // Store the selected value
          input.value = selectedValue;
          closeAllLists();
        }
      }
...
$(document).ready(function() {
    ...
    $("#clearSearch").click(function() {
        $("#searchInputDashboard").val(""); // Clear the input field
        $("#autocompleteSuggestions").empty(); // Clear autocomplete suggestions
    });
...
map.on("contextmenu", async (e) => {
  ...
  for (let layer in layerDetails) {
    ...
    let urrr = `${main_url}pmc/wms?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetFeatureInfo&FORMAT=image%2Fpng&TRANSPARENT=true&QUERY_LAYERS=${layer}&STYLES&LAYERS=${layer}&exceptions=application%2Fvnd.ogc.se_inimage&INFO_FORMAT=application/json&FEATURE_COUNT=50&X=${Math.round(
      e.containerPoint.x
    )}&Y=${Math.round(e.containerPoint.y)}&SRS=EPSG%3A4326&WIDTH=${
      size.x
    }&HEIGHT=${size.y}&BBOX=${bbox}&CQL_FILTER=${cqlFilter123}`;
