...
var drawControlElectrical = new L.Control.Draw({
  draw: {
    polyline: {
      shapeOptions: {
...
if (workType == "New") {
  ...
  customDrawControls.onAdd = function (map) {
    ...
    div.innerHTML =
      '<button class="draw_feature"  style=" " title="Draw New Feature"> <img src="png/006-drawing.png" style="width: 20px; height: 20px; padding:0px 3px;"></button>';
...
customEditLayerButton.onAdd = function (map) {
  ...
  div.innerHTML =
    '<img id="editFeatureButton"  title="Edit Feature" src="png/editTool.png" style="width: 20px; height: 20px; padding:0px 3px;">';
  customDrawControlsContainer = div;
  return div;
};
...
function getClosestRoadPointLast(latlng) {
  ...
  var url = `https://pmc.geopulsea.com/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=${layer}&outputFormat=application/json&bbox=${bbox.join(
    ","
  )},EPSG:4326`;
...
function Savedata(lastDrawnPolylineId) {
  ...
  formData.append(
    "latitude",
    selectCoordinatesData[selectCoordinatesData.length - 1].geometry
      .coordinates[0][1]
  );
  formData.append(
    "longitude",
  ...
  formData.append(
    ...
    JSON.stringify(
      selectCoordinatesData[
        selectCoordinatesData.length - 1
